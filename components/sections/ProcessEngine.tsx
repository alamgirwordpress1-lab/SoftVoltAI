 ...
 */
export function ProcessEngine({ steps, copy }: { steps: ProcessStep[]; copy: LinkedHeadingCopy & { artefacts: { title: string; meta: string }[] } }) {
            ...
            {steps.map((s, i) => (
              ...
              >
                ...
                <div className="mt-6 lg:hidden">
                  <Artefact kind={s.artefact} card={copy.artefacts[i]} />
                </div>
              </li>
            ))}
          </ol>
...
function DocCard({ title, meta, children }: { title: string; meta: string; children: React.ReactNode }) {
  return (
...
function Artefact({ kind, card }: { kind: string; card?: { title: string; meta: string } }) {
  switch (kind) {
    case "brief":
      return (
        <DocCard title={card?.title || "Brief received"} meta={card?.meta || "northwind-digital · 17:04 London"}>
    ...
    case "scope":
      return (
        <DocCard title={card?.title || "Scope & quote"} meta={card?.meta || "v1 · within 2 business days"}>
    ...
    case "staging":
      return (
        <DocCard title={card?.title || "Production"} meta={card?.meta || "staging.northwind-digital.co.uk"}>
    ...
    case "qa":
      return (
        <DocCard title={card?.title || "QA checklist"} meta={card?.meta || "42 checks · published"}>
    ...
    default:
      return (
        <DocCard title={card?.title || "Handover"} meta={card?.meta || "northwind-digital · launch day"}>
